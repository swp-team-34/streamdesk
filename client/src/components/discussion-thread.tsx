...
export function DiscussionThread({
  ...
  return (
    <div className="space-y-4">
      {!isConnected && (
        <div className="rounded-surface border border-warning/20 bg-warning-muted/60 px-3 py-2 text-xs text-warning">
          Realtime временно недоступен. Отправка продолжит работать через HTTP, после подключения данные обновятся.
        </div>
      )}
      {subscriptionDenied && (
        <div className="rounded-surface border border-error/20 bg-error-muted/60 px-3 py-2 text-xs text-error">
          Realtime-подписка недоступна для этого обсуждения. Проверьте права доступа.
        </div>
      )}

      {commentsQuery.isLoading ? (
        <div className="rounded-surface border border-dashed border-border/60 bg-muted/20 px-4 py-8 text-center text-sm text-muted-foreground">
          Загружаем обсуждение...
        </div>
      ) : commentsQuery.isError ? (
        <div className="space-y-2 rounded-surface border border-error/20 bg-error-muted/40 p-4 text-sm">
      ...
      {canComment ? (
        <div className="space-y-2 border-t border-border/40 pt-4">
          {replyRoot && (
            <div className="flex items-center justify-between gap-3 rounded-control bg-muted/40 px-3 py-2 text-xs">
