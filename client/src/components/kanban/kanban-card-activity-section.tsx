    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-3">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {entries.length > 0 && (
            <Badge variant="outline" className="rounded-full border-border/40 bg-surface-subtle text-xs text-muted-foreground">
              {entries.length}
            </Badge>
          )}
        </div>
      ...
      {entries.length === 0 ? (
        ...
        <div className="space-y-3">
          {visibleEntries.map((entry) => {
            ...
            return (
              <div key={entry.id} className="rounded-surface border border-border/50 bg-surface-subtle p-4">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="text-sm font-medium">
                    {getUserName(entry.userId)}
                  </div>
                  <div className="rounded-full border border-border/40 bg-surface-overlay px-2 py-1 text-xs text-muted-foreground">
                ...
                {changeLines.length > 0 && (
                  <div className="mt-3 space-y-2">
                    {changeLines.map((line, index) => (
                      <div
                        key={`${entry.id}-${index}`}
                        className="rounded-control border border-border/50 bg-surface-raised px-3 py-2 text-sm"
                      >
                        {line}
                      </div>
                    ))}
